...
export function ClaimsSummary({ initialClaims = [] }: { initialClaims?: Claim[] }) {
  ...
  const summary = useMemo(() => {
    ...
    const statementsNeeded = safeClaims.filter((claim) => !claim.statementMailed && claim.patientResponsibility > 0).length;
    const statementsSent = safeClaims.filter((claim) => claim.statementMailed).length;
    const totalOutstanding = safeClaims
      .filter((claim) => !claim.statementMailed)
      .reduce((acc, claim) => acc + claim.patientResponsibility, 0);
    const totalInsurancePaid = safeClaims.reduce((acc, claim) => acc + claim.insurancePaid, 0);
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      ...
      <Card className="relative overflow-hidden border-none bg-white/70 p-[1px] shadow-lg shadow-sky-200/50 backdrop-blur-xl transition hover:scale-[1.01]">
        <div className="rounded-2xl bg-gradient-to-br from-white/80 via-white/70 to-white/50 p-6">
          ...
          <CardContent className="px-0">
            <div className="text-3xl font-bold text-slate-800">
              {formatCurrency(summary.totalInsurancePaid)}
            </div>
